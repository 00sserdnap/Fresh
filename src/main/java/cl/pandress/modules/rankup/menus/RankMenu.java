...
public class RankMenu {
            ...
            for(String line : config.getStringList(pathMenu + "lore")) {
                if (line.contains("%rewards%")) {
                    ...
                    if (config.contains(pathRank + ".requirements.money")) {
                        ...
                        double bal = manager.getEconomy() != null ? manager.getEconomy().getBalance(player) : (double)0.0F;
    ...
    private static ItemStack getTopItem(RankManager manager, FileConfiguration config) {
        ...
        for(Map.Entry<UUID, Integer> entry : manager.getTopRanks()) {
            ...
            finalLore.add(ChatUtils.colorize(format.replace("%pos%", String.valueOf(pos)).replace("%player%", name).replace("%rank%", String.valueOf(entry.getValue()))));
            ++pos;
        }
    ...
    private static String formatNumber(double value) {
        if (value >= (double)1.0E9F) {
            return df.format(value / (double)1.0E9F) + "b";
        } else if (value >= (double)1000000.0F) {
            return df.format(value / (double)1000000.0F) + "m";
        } else {
            return value >= (double)1000.0F ? df.format(value / (double)1000.0F) + "k" : df.format(value);
        }
    }
}
